...
export async function execute(interaction: ChatInputCommandInteraction) {
  try {
    ...
    if (betChoice === null) {
      await interaction.editReply({ embeds: [errorEmbed("Invalid bet! Use: `red`, `black`, `green`, `odd`, `even`, `low`, `high`, or a number 0-36.")] });
      return;
    }
    ...
    console.error("[Roulette Error]", err?.message ?? err);
    try {
