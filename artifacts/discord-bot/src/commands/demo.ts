...
export async function execute(interaction: ChatInputCommandInteraction) {
  ...
  if (user.lastDemo) {
    ...
    if (diff < DEMO_MS) {
      ...
      await interaction.reply({ embeds: [errorEmbed(`You already claimed demo cash! Come back in **${h}h ${m}m ${s}s**.\n\nCurrent demo balance: ${formatRobux(user.demoBalance)} Demo Robux`)], ephemeral: true });
      return;
    }
  }
