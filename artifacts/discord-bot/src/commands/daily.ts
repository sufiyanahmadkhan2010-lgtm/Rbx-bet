...
export async function execute(interaction: ChatInputCommandInteraction) {
  ...
  if (user.lastDaily) {
    ...
    if (diff < DAILY_MS) {
      ...
      await interaction.reply({ embeds: [errorEmbed(`You already claimed your daily! Come back in **${h}h ${m}m ${s}s**.`)], ephemeral: true });
      return;
    }
  }
  ...
  await interaction.reply({ embeds: [winEmbed("Daily Claimed!", `You received ${formatRobux(DAILY_AMOUNT)}!\nNew balance: ${formatRobux(updated.balance)}`)] });
}
